...
export function ManageClients({ onToast }: Props) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      onToast('Client deleted successfully', 'success');
      setConfirmId(null);
    } catch {
      onToast('Failed to delete client — please try again', 'error');
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="h-full overflow-y-auto custom-scrollbar px-6 py-6">
      <div className="max-w-4xl mx-auto space-y-5">
        <div className="mb-5">
          <h3 className="text-[#EEF3FA] font-bold text-sm mb-0.5" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>Manage Clients</h3>
          <p className="text-[11px] text-[#7A94B4]">View all clients and permanently remove them from the system.</p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[11px] min-w-[640px]">
              ...
              <tbody>
                {clients.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-8 text-center text-[#7A94B4] text-[11px]">No clients found.</td>
                  </tr>
                )}
                {clients.map(client => (
