...
export function RiskMatrix({ risks }: { risks: Risk[] }) {
  ...
  return (
    <div className="h-[330px] rounded-xl border border-[#1C3050] bg-[#09152A] p-3">
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart margin={{ top: 12, right: 18, bottom: 18, left: 0 }}>
          ...
          <XAxis type="number" dataKey="x" domain={[1, 5]} ticks={[1, 2, 3, 4, 5]} tick={{ fill: '#5A6E88', fontSize: 10 }} label={{ value: 'Impact', position: 'insideBottom', fill: '#5A6E88', fontSize: 10 }} />
          <YAxis type="number" dataKey="y" domain={[1, 5]} ticks={[1, 2, 3, 4, 5]} tick={{ fill: '#5A6E88', fontSize: 10 }} label={{ value: 'Probability', angle: -90, position: 'insideLeft', fill: '#5A6E88', fontSize: 10 }} />
          ...
          <Tooltip
            ...
            content={({ active, payload }) => {
              ...
              return (
                <div className="max-w-[260px] rounded-lg border border-[#264468] bg-[#09152A] p-3 text-xs text-[#F0F4FF] shadow-xl">
                  <div className="font-bold">{risk.title}</div>
                  <div className="mt-1 text-[#BCC8DC]">{risk.mitigation}</div>
                </div>
              );
            }}
          />
